...
<!--<div id="mainMenu" class="clearfix">
  ...
  <div class="btn-toolbar pull-right">
    ...
    <?php common::printLink('contract', 'createinvoice', '', "<i class='icon-plus'></i> " . $lang->invoice->create, '', "class='btn btn-primary'") ?>
  </div>
</div>
      ...
      <table class="table has-sort-head table-fixed" id='productList'>
        ...
        <thead>
          <tr>
            ...
            <th class='w-70px' title='<?php echo $lang->invoice->amount;?>'><?php echo $lang->invoice->amount;?></th>
            <th class='w-110px' title='<?php echo $lang->invoice->submitteddate;?>'><?php echo $lang->invoice->submitteddate;?></th>
            <th class='w-60px' title='<?php echo $lang->invoice->step;?>'><?php echo $lang->invoice->step;?></th>
            <th class='w-70px' title='<?php echo $lang->invoice->edit;?>'><?php echo $lang->invoice->edit;?></th>
            <?php if($canOrder):?>
        ...
        <tr data-id='<?php echo $invoice->id ?>' data-order='<?php echo $invoice->id;?>'>
         ...
         <!-- <td class="c-name" title='<?php echo $invoice->contractID?>'><?php echo html::a($this->createLink('contract', 'invoiceview', "contract=" . $invoice->id."-"."$invoice->contractID" ), $invoice->contractID );?></td>--> <!-- changed the hyperlink to invoiceview 2020.1.10-->
          ...
          <td><?php common::printIcon('contract', 'edit', "invoiceID=$invoice->id", $id, 'list');?></td>
          <?php if($canOrder):?>
